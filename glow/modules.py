...
class _ActNorm(nn.Module):
    ...
    def __init__(self, num_features, scale=1.):
        ...
        # self.inited = nn.Parameter(torch.tensor(False, dtype=torch.bool), requires_grad=False)
        self.inited = nn.Parameter(torch.tensor(0), requires_grad=False)
    ...
    def forward(self, input, logdet=None, reverse=False):
        if not self.inited:
            ...
            self.inited = torch.nn.Parameter(torch.tensor(1.0).to(self.bias.device), requires_grad=False)
        self._check_input_dim(input)
...
class Conv2dNormal(nn.Conv2d):
    def __init__(self, in_channels, out_channels, 
        ...
        super().__init__(in_channels, out_channels, kernel_size, stride, padding)
        self.logscale_factor = logscale_factor
        logsinit = np.random.normal(loc = 0.0, scale = 1e-3, size = (out_channels, 1, 1))
        logsinit = torch.from_numpy(logsinit)
        logsinit = logsinit.float()
        self.register_parameter("logs", nn.Parameter(logsinit))
        # init
        self.weight.data.normal_(0.0, 1e-3)
        self.bias.data.normal_(0.0, 1e-3)
...
class InvertibleConv1x1(nn.Module):
    ...
    def forward(self, input, logdet=None, reverse=False):
        ...
        # pw debug: make sure input is float
        input = input.float()
        if not reverse:
