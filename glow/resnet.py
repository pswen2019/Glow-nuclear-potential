...
class NN2dBlock(nn.Module):
    def __init__(self, 
        ...
        if use_batch_norm:
            self.batch_norm_layers = nn.ModuleList([
                nn.BatchNorm2d(features, eps = 1e-3)
                for _ in range(2)
            ])

        self.conv2d_layer = nn.ModuleList([
        ...
        self.dropout = nn.Dropout(p = dropout_probability)
        if zero_initialization:
            init.uniform_(self.conv2d_layer[-1].weight, -1e-3, 1e-3)
            init.uniform_(self.conv2d_layer[-1].bias, -1e-3, 1e-3)
    ...
    def forward(self, inputs, context = None):
        ...
        if self.use_batch_norm:
            temps = self.batch_norm_layers[0](temps)
        temps = self.activation(temps)
        temps = self.conv2d_layer[0](temps)

        if self.use_batch_norm:
            temps = self.batch_norm_layers[1](temps)
        temps = self.activation(temps)
        temps = self.dropout(temps)
        temps = self.conv2d_layer[1](temps)
        # todo: define the nn for context
        return inputs + temps

class NN2dNet(nn.Module):
    def __init__(self, 
                 in_features, 
                 out_features, 
                 hidden_features, 
