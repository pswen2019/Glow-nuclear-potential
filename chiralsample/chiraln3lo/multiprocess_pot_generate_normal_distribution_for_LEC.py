...
lecn3lo450 = np.loadtxt("Chiral2017LEC_N3LO450.dat", unpack = True)
lecn3lo500 = np.loadtxt("Chiral2017LEC_N3LO500.dat", unpack = True)
lecn3lo550 = np.loadtxt("Chiral2017LEC_N3LO550.dat", unpack = True)
lecn3 = [lecn3lo450, lecn3lo500, lecn3lo550]
diff450to500 = np.abs(lecn3lo450 - lecn3lo500)
diff450to550 = np.abs(lecn3lo450 - lecn3lo550)
diff500to550 = np.abs(lecn3lo500 - lecn3lo550)

lecmax = np.max([lecn3lo450, lecn3lo500, lecn3lo550], axis = 0)
lecmin = np.min([lecn3lo450, lecn3lo500, lecn3lo550], axis = 0)
...
def generatepot(processnum, numpot):
    ...
    for i in tqdm(range(numpot), ncols = 54):
        ...
        leclist = np.random.normal(loc = lecn3[potid], scale = width)
        leclist[1] = leclist[3]

        leclist[-1] = lecn3[potid][-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--numprocess', type = int, default = 100, 
    ...
    if os.path.exists('a.out'):
        os.remove('a.out')
    subprocess.call(['ifort', 'Chiral2017_N3LO_PotGen.f', '-O3'])
