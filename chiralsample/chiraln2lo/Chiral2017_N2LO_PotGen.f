...
!     Momentum array dimensions
!      parameter ( ndim = 128 )
      parameter ( ndim = 48 )
!      parameter ( ndim = 32 )

!     Max J and max momentum
      parameter ( jmax1 = 7 )

      dimension xk(ndim),vkk(3,jmax1,6,ndim,ndim)

      character * (256) sord,slamb,spot,pot1,pot2,pot3
...
c     EXPLANATIONS ARE GIVEN BELOW AND AT THE BEGINNING OF CDBONN.
      ...
      ! pw: load LEC
      dimension xlec(12)
      common /clecs/ xlec, xlamb, jnn

      ! pw: lec related channels
      dimension j0channel(2), j1channel(6)
      dimension j2channel(5), j3channel(1)
      data j0channel /1, 3/
      data j1channel /1, 2, 3, 4, 5, 6/
      data j2channel /1, 2, 4, 5, 6/
      data j3channel /4/
...
!     Some constants
      ...
      hbarc = 197.327d0
      xmnuc = 938.9182d0 / hbarc

      ! pw: load LECs from files 
      open(233, file = "lec.dat", 
     &   status = "old", 
     &   form = "formatted")
      ...
      open(66, file = "Momenta.dat", status = "unknown", 
     &   form = "formatted")

      pot2 = 'pot/pot.dat'
      open(22, file = pot2,  status = 'unknown', form = 'formatted')


!       call meshbuild(xk, 300.d0, ndim, 100, ndim - 100, 2.5d0)
      call meshbuild(xk, 300.d0, ndim, 32, ndim - 32, 2.5d0)
!      call meshbuild(xk, 300.d0, ndim, 21, ndim - 21, 2.3d0)
      write(66, '(500e14.7)' )( xk(ncol), ncol = 1, ndim )
         ...
         do j1 = 1, jmax1
            ...
            do ix = 1, ndim
               ...
               do iy = 1, ndim
                  ...
                  do icoup = 1, 6
                     vkk(ipn,j1,icoup,ix,iy) = v(icoup) 
c     &                * hbarc ** 2 * xmnuc
                  enddo
      ...
      ! pw: record lec related partial-wave channels
      ! write J = 0 
      do ichannel = 1, 2 
         ipn = 2
         jid = 1
         idchannel = j0channel(ichannel)
         do nrow = 1, ndim
            write( 22, '(500e14.6)' )( 
     &            vkk(ipn,jid,idchannel,nrow,ncol), 
     &            ncol = 1, ndim )
         enddo
      enddo
...
!     PART 1 END: Main program
!     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine meshbuild(xk, xelab, ndim, npoint, ninfpoint, power)
      implicit double precision (a-h, o-z)
      dimension xk(ndim)
      ...
      xkpole = ( 
     &    xmp ** 2 * xelab * ( xelab + 2.0 * xmn ) / 
     &    ( ( xmp + xmn ) ** 2 + 2.0 * xelab * xmp ) 
     &    ) ** (1.d0/2.d0)
      xkpole = xkpole / 197.327d0
      xkmax = 2.d0 * xkpole
      
      call finitemeshbuild(xk, xkmax, ndim, npoint, power)
      call infmeshbuild(xk, ndim, npoint, ninfpoint)

      end subroutine meshbuild


      subroutine finitemeshbuild(xk, xkmax, ndim, npoint, power)
      implicit double precision (a-h, o-z)
      dimension xk(ndim)
      integer npoint

      xstart = 0.001d0
      xdelta = (xkmax - xstart)/(npoint - 1)
      do i = 1, npoint
         xkk = xstart + (i - 1) * xdelta
         xkf = ((xkk - xstart)/(xkmax - xstart))**power
         xk(i) = xkf * (xkmax - xstart) + xstart
      enddo

      end subroutine finitemeshbuild

      subroutine infmeshbuild(xk, ndim, npoint, ninfpoint)
      implicit double precision (a-h, o-z)
      dimension xk(ndim), xktemp(ndim)
      
      pi    = 3.141592653589793d0

      xmax = xk(npoint)
      dydx = (3.d0*xk(npoint)/2.d0 
     &        - 2.d0*xk(npoint-1) 
     &        +xk(npoint-2)/2.d0)

      do i = 1, ninfpoint 
         xx = (pi/2.d0) * ( (i*1.d0) / (ninfpoint + 0.5d0))
         xktemp(i) = TAN(xx)
      enddo 

      dtandx = ( - 3.d0*xktemp(1)/2.d0 
     &           + 2.d0*xktemp(2) 
     &           - xktemp(3)/2.d0)

      do i = npoint + 1, npoint + ninfpoint
         idtemp = i - npoint
         xk(i) = (dydx / dtandx) * xktemp(idtemp) + xmax
      enddo 
         
      end subroutine infmeshbuild
...
c
      ...
      common /clecs/ xlec, xlamb, jnn
      dimension lecid(9)
      data lecid /26, 27, 30, 32, 34, 36, 37, 41, 46/
